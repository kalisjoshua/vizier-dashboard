...
export class Overview extends HTMLDivElement {
  static #element;

  constructor() {
    super();

    Overview.#element = inDOM(`[is="${NAME}"]`);
  }

  static render(org, repoConfigs) {
    Overview.#element.innerHTML = Object.entries(repoConfigs)
      .map(([repo, { sonar }]) => {
  ...
  static showPanelContents() {
    Overview.#element.parentNode.setAttribute("open", true);
  }
}

customElements.define(NAME, Overview, { extends: "div" });
